	...
	var Resumo = {
		...
		modal: function(){
			...
			item.on('click',function(){
				...
				// Initialize blog slider if it exists (after BgImg is called)
				setTimeout(function(){
					if(modalBox.find('.blog_slider').length){
						self.blogModalSlider(modalBox);
					}
				}, 50);
				...
				if(name === ''|| email === ''|| message === ''){
					...
					$.ajax({
						...
						error: function(xhr, status, error) {
							// Formspree returns 200 on success with JSON, but sometimes redirects
							...
							if(xhr.status === 200 || xhr.status === 0){
								...
								// Formspree returned JSON response
								if(xhr.responseJSON.error) {
									spanSuccess.append("<span class='contact_error'>" + xhr.responseJSON.error + "</span>");
								} else if(xhr.responseJSON.ok || xhr.status === 200) {
									spanSuccess.append("<span class='contact_success'>" + success + "</span>");
									spanSuccess.slideDown(500).delay(4000).slideUp(500);
									form[0].reset();
									$('.input_wrapper').removeClass('active');
								} else {
									spanSuccess.append("<span class='contact_error'>There was an error sending your message. Please try again.</span>");
								}
								spanSuccess.slideDown(500).delay(4000).slideUp(500);
							} else {
								...
								try {
									...
									// If we can't parse, check status code
									if(xhr.status === 200 || xhr.status === 302) {
										// Likely success (redirect)
										spanSuccess.append("<span class='contact_success'>" + success + "</span>");
										spanSuccess.slideDown(500).delay(4000).slideUp(500);
										form[0].reset();
										$('.input_wrapper').removeClass('active');
		...
		blogModalSlider: function(modalBox){
			var owl = modalBox.find('.blog_slider');
			owl.each(function(){
				var el = $(this);
				...
				if(el.data('owl.carousel')){
					...
					el.find('.owl-nav').remove();
					el.find('.owl-dots').remove();
				}
				
				// Set background images before initializing carousel
				el.find('[data-bg-img]').each(function(){
					var element = $(this);
				...
				// Check if this is the RiteSize slider (should show 1 item per view)
				var isRiteSize = el.hasClass('ritesize_slider');
				var carouselConfig = {
					autoplay: true,
					autoplayTimeout: 5000,
					smartSpeed: 1000,
					margin: isRiteSize ? 0 : 20,
					nav: true,
					loop: true,
					items: isRiteSize ? 1 : 3,
					dots: true,
				...
				// Add responsive breakpoints only if not RiteSize slider
				if(!isRiteSize){
					carouselConfig.responsive[768] = {
						items: 2,
						margin: 20
					};
					carouselConfig.responsive[1024] = {
						items: 3,
						margin: 20
					};
				}
				
				el.owlCarousel(carouselConfig);
				el.trigger('refresh.owl.carousel');
				
				// Set background images again after carousel is initialized
				setTimeout(function(){
					el.find('[data-bg-img]').each(function(){
						var element = $(this);
						var attrBg = element.attr('data-bg-img');
